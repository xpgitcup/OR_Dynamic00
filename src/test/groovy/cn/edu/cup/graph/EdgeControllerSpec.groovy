...
class EdgeControllerSpec extends Specification {
    ...
    void "Test the index action returns the correct model"() {
        given:
        controller.edgeService = Mock(EdgeService) {
            1 * list(_) >> []
            1 * count() >> 0
        }
    ...
    void "Test the save action with an invalid instance"() {
        given:
        controller.edgeService = Mock(EdgeService) {
            1 * save(_ as Edge) >> { Edge edge ->
                throw new ValidationException("Invalid instance", edge.errors)
            }
        }
